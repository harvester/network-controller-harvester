...
func (h Handler) OnChange(_ string, cn *networkv1.ClusterNetwork) (*networkv1.ClusterNetwork, error) {
	...
	// for mgmt cluster network, add mgmt vid to list first, to avoid it to be on the following removedList
	if utils.IsManagementClusterNetwork(cn.Name) && h.mgmtVlan > utils.DefaultVlanID {
	...
	*/
	addedList := added.ToLocalAreas()
	removedList := removed.ToLocalAreas()
