...
func NewVlanIDSetFromNetConf(nc *NetConf, cidr string) (*VlanIDSet, error) {
	if nc == nil {
...
func NewVlanIDSetFromNadList(nads []*nadv1.NetworkAttachmentDefinition) (*VlanIDSet, error) {
	...
	for _, nad := range nads {
		...
		l3conf, err := NewLayer3NetworkConfFromNad(nad)
		if err != nil {
			return nil, err
		}

		tmpvis, err := NewVlanIDSetFromNetConf(nc, l3conf.CIDR)
		if err != nil {
			return nil, err
		}
