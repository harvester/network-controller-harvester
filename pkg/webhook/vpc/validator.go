...
func (v *Validator) Create(_ *admission.Request, newObj runtime.Object) error {
	...
	if err != nil {
		return fmt.Errorf("failed to validate vpc spec config for vpc %s in %s", vpc.Name, vpc.Namespace)
	}
...
func (v *Validator) Update(_ *admission.Request, oldObj, newObj runtime.Object) error {
	...
	if err != nil {
		return fmt.Errorf("failed to validate vpc spec config for vpc %s in %s", newVpc.Name, newVpc.Namespace)
	}
...
func (v *Validator) Delete(_ *admission.Request, oldObj runtime.Object) error {
	...
	for _, subnet := range subnets {
		if subnet.Spec.Vpc == vpc.Name {
			return fmt.Errorf("delete the subnets associated with vpc %s in %s", vpc.Name, vpc.Namespace)
		}
	}
